...
int parsecmd(char* __buf, int __bufsize, struct cmdlist* __head)
{
  ...
  while((word = strtok(cmd, " \t\n")) != NULL){ 
    if( strcmp(word, "||") == 0 || strcmp(word, "&&") == 0){
      struct cmdlist* newCurr = (struct cmdlist*)malloc(sizeof(struct cmdlist));
      curr->next = newCurr;
      ...
      curr->conjuction = strcmp(word, "||") ? CONJAND : CONJOR;
      continue;
    }
...
int executecmds(struct cmdlist* __head)
{
  ...
  while(curr != NULL){
    ...
    if(  
      ...
      (curr->conjuction == CONJOR && procres == 0) ||
      (curr->conjuction == CONJAND && procres == 1)
    ){
      f = fork();
      e = errno;
      ...
      waitpid(f, &wstatus, 0);
      procres = WEXITSTATUS(wstatus) ? 0 : 1;
      printf("Exit code:%d \n", WEXITSTATUS(wstatus));
    }
