...
int main(void)
{
  ...
  while(1) {
    ...
    fcntl(fdcnt, F_SETFL, fcntl(fdcnt, F_GETFL, 0) & ~O_NONBLOCK); // Remove O_NONBLOCK
    fdcntp = open(fifocntname, O_WRONLY); // Open FIFO in O_WRONLY mode just not to crash read system call
