...
int main(void)
{
  ...
  fcntl(fdsrv, F_SETFL, fcntl(fdsrv, F_GETFL, 0) & ~O_NONBLOCK); // Remove O_NONBLOCK
  fdsrvp = open(fifosrvname, O_WRONLY); // Open FIFO in O_WRONLY mode just not to crash read system call
  if(fdsrv == -1){
    printf("FAIL!\nError: \'%s\'\n", strerror(errno));
    return 0;
  }
